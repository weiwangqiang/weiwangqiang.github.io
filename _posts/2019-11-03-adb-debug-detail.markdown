...
| `task [COMMAND] [...]`                                       | 用于操作activity task的子命令。<br>具体command如下：<br><br>`lock <TASK_ID>` : 将TASK_ID置为前台，并且不让其他stask工作（转为前台）,即锁上TASK_ID<br><br>`lock stop` : 将当前的task 锁解锁 <br><br>`resizeable <TASK_ID> ` |
